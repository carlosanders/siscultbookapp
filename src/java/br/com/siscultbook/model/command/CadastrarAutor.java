 ...
 * @author Carlos
 */
public class CadastrarAutor implements InterfaceCommand {

    private InterfaceAutorDAO autorDAO;

    public CadastrarAutor(InterfaceAutorDAO autorDAO) {
        super();
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        request.setAttribute("titulo", "Cadastro - Autor(a)");
        ...
        try {
            ...
            if (autorSobrenome.equals("") || autorSobrenome == null || autorSobrenome.length() < 3) {
                mensagem = mensagem + "O campo sobrenome deve ser maior que 3 caracteres.<br />";
                retorno = true;
            } else {
            ...
            if (retorno) {
                ...
                if (autorDAO.buscarAutorPorCPF(cpf) == null) {
                    /* envio as expression laguage do funcionario para página através dos request abaixo */
                    request.setAttribute("autorDataNascimento", Utilitario.DataFormatada(autor.getDataNascimento()));
                    request.setAttribute("autorSexo", autor.getSexo());
                    request.setAttribute("autorEstado", autor.getEndereco().getEstado());
                    ...
                    request.setAttribute("mensagem", "Autor(a) (" + autor.getNomeCompleto() + ") gravado com sucesso!");
                } else {
                    /* envio as expression laguage do funcionario para página através dos request abaixo */
                    request.setAttribute("autorDataNascimento", Utilitario.DataFormatada(autor.getDataNascimento()));
                    request.setAttribute("autorSexo", autor.getSexo());
